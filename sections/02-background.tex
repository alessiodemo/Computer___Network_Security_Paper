...
\paragraph{Namespaces.} Linux namespaces partition global resources into per-process views \cite{linux_namespaces_manpage}. The key namespaces for container isolation are: \texttt{pid} (process identifiers), \texttt{net} (network stack), \texttt{mnt} (filesystem mount points), \texttt{uts} (UNIX time-sharing System), \texttt{ipc} (inter-process communication), and \texttt{user} (user and group identifiers). A process confined within these namespaces cannot observe or interact with resources belonging to other namespaces.

\paragraph{Control Groups (cgroups).} Cgroups enforce resource accounting and limit CPU time, memory, I/O bandwidth, and device access. They do not provide security isolation, but prevent resource exhaustion attacks and can restrict device access via the \texttt{devices} cgroup controller.
...
These primitives form a defence-in-depth stack \cite{lin2018measurement} \cite{nist_sp800_190}, however their security guarantees are only as strong as the correctness of the kernel implementation and the integrity of the privileged processes that configure and manage the container lifecycle.
...
The NVIDIA Container Toolkit is an open-source project that enables NVIDIA GPU access for containerised workloads \cite{nvidia_ctk_github}. The Container NVIDIA Toolkit stack includes several components:
\begin{itemize}
  \item NVIDIA Container runtime (\textbf{nvidia-container-runtime}): Takes a \texttt{runc} specification as input, injects the NVIDIA container runtime hook and then calls out to the native \texttt{runc}, passing it the modified \texttt{runc} specification with that hook set.
  \item NVIDIA Container runtime hook (\textbf{nvidia-container-toolkit} / \textbf{nvidia-container-runtime-hook}): the hook is included in the \textbf{nvidia-container-toolkit} package. The hook is invoked by \texttt{runc} after a container has been created, but before it has been started, and is given access to the \texttt{config.json} associated with the container.
  It then takes information contained in the config.json and uses it to invoke the nvidia-container-cli CLI with an appropriate set of flags
  \item NVIDIA Container Library and CLI (\textbf{libnvidia-container1} and \\ \textbf{nvidia-container-cli}): responsible to provide a library and command-line interface to configure Linux containers leveraging NVIDIA GPUs.
\end{itemize}
The toolkit integrates with the Open Container Initiative (OCI) specification through the \textbf{OCI hook} mechanism \cite{oci_runtime_spec} \cite{oci_image_spec}. OCI hooks are executable programs invoked by the container runtime at defined lifecycle points (\texttt{createRuntime}, \texttt{createContainer}, \texttt{startContainer}, \texttt{poststart}, and \texttt{poststop}); the older \texttt{prestart} hook was deprecated in OCI Runtime Spec v1.1 in favour of these more granular alternatives. The NVIDIA Container Toolkit registers a \texttt{createContainer} hook that runs as a privileged process \textit{on the host}, before the container's \texttt{pivot\_root} operation but after its mount namespace has been created; its role is to mount the necessary NVIDIA device files and libraries into the container's filesystem \cite{nvidia_ctk_docs}.
...
\begin{infobox_env}
The OCI hook specification (\textit{runtime-spec}) does not mandate environment sanitisation for hook executables \cite{oci_runtime_spec}. The security burden is implicitly placed on hook implementers, who must explicitly clean their execution environment when operating with elevated privileges.
\end{infobox_env}
...
However, this protection is only automatic for Set-UID binaries; for a process that acquires elevated privileges through other means (such as being spawned directly by a privileged parent) and does not explicitly sanitize its environment, \texttt{LD\_PRELOAD} remains fully operative \cite{linux_execve_manpage}. This is the precise gap that NVIDIAScape exploits.
