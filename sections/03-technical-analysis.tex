...
Figure~\ref{fig:nvidia-container} illustrates the attack flow.
...
\begin{enumerate}
  \item \textbf{Craft the malicious shared library.} The attacker writes a shared library with a constructor function (a function annotated with \texttt{\_\_attribute\_\_((constructor))} in C) that executes the attacker's payload when the library is loaded. Because this runs before \texttt{main()}, it executes at the earliest possible moment in the host process lifecycle, before any security checks in the hook's application logic.
...
\end{lstlisting}
  ...
  \item \textbf{Hook invocation.} As part of the container startup sequence, the OCI runtime invokes the NVIDIA \texttt{createContainer} hook (nvidia-ctk hook \texttt{enable-cuda-compat}) on the host with root privileges.
  \\ Due to the environment inheritance vulnerability, the crafted library is present in the hook's environment.
